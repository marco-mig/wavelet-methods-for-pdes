function solution = wav_sturm_liouville(J)
# Approximates the solution of the sturm-liouville problem:
# -u''(x) + c*u(x) = f(x) 
...
# The level J is given, and the calculation is done for the given c and RHS

c = pi^2;
f = @(x) (4*pi^2+c)*sin(2*pi*x);
...
# Periodize 
for i=1:length(dv)
  dv{i} = @(x) dv{i}(x) + dv{i}(x-1) + dv{i}(x+1) + dv{i}(x+2) + dv{i}(x-2);
endfor 
...
g = @(x) (phi(x))^2;
h = @(x) (phid(x))^2;
A(1,1) = c*(1/2)*(g((1/2)*(-3^(1/2)/3+1)) + g((1/2)*(3^(1/2)/3+1))) + (1/2)*(h((1/2)*(-3^(1/2)/3+1)) + h((1/2)*(3^(1/2)/3+1)));
for j=0:J
  for k=0:2^j-1 
    for l=0:J
      for m=0:2^l-1 
        ...
        for i=1:length(grid)-1 
          d1 = grid(i);
          d2 = grid(i)+q;
          inta = inta + (d2-d1)/2 * (g((1/2)*((-(3^(1/2))/3)*(d2-d1) + d2 + d1)) + 
          g((1/2)*(((3^(1/2))/3)*(d2-d1) + d2 + d1)));
        endfor
...
for j=0:J
  for k=0:2^j-1
    ...
    for i=1:length(grid)-1
      d1 = grid(i);
      d2 = grid(i)+q;
      intb = intb + (d2-d1)/2 * (g((1/2)*((-(3^(1/2))/3)*(d2-d1) + d2 + d1)) +
          g((1/2)*(((3^(1/2))/3)*(d2-d1) + d2 + d1)));
    endfor
    ...
    A(1, 2^j+k+1) = c*intb + int2;
    A(2^j+k+1, 1) = c*intb + int2;
  endfor
endfor  
